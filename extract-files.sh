...
HELPER="$MORPHOS_ROOT"/vendor/morphos/build/tools/extract_utils.sh
if [ ! -f "$HELPER" ]; then
...
setup_vendor "$DEVICE_COMMON" "$VENDOR" "$MORPHOS_ROOT" true "$CLEAN_VENDOR"
...
if [ -s "$MY_DIR"/../$DEVICE/proprietary-files.txt ]; then
    ...
    setup_vendor "$DEVICE" "$VENDOR" "$MORPHOS_ROOT" false "$CLEAN_VENDOR"

    extract "$MY_DIR"/../$DEVICE/proprietary-files.txt "$SRC" "$SECTION"
fi

COMMON_BLOB_ROOT="$MORPHOS_ROOT"/vendor/"$VENDOR"/"$DEVICE_COMMON"/proprietary
